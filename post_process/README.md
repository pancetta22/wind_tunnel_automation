...
| `flutter_analysis.py`        | **フラッター実験 後処理 入口**。→ [FLUTTER_README.md](FLUTTER_README.md) |
| `lco_analysis.py`            | フラッターの LCO 非線形解析。`--lco` で有効化 → [FLUTTER_README.md](FLUTTER_README.md) |
...
`--temp`/`--pressure` 等は省略すると experiment_log.json から自動取得）

| make_windspeed.py オプション | 必須  | 説明                                                          |
| ----------------------------- | :---: | -------------------------------------------------------------- |
| `--volt_dir`                  |   ✔   | `volt_summary.csv` があるフォルダ                              |
| `--date`                      |   ✔   | 実験日 `YYYYMMDD`                                              |
| `--log`                       |       | `experiment_log.json` のパス（省略時: volt_dir 内を自動検索）  |
| `--temp`                      |       | 気温 [℃]（`--log` より優先）                                   |
| `--pressure`                  |       | 気圧 [mmHg]（`--log` より優先）                                |
...
差圧電圧 V [mV]・気温 T [℃]・気圧 P [mmHg] から：

```
飽和水蒸気圧  e     = 6.1078 × 10^(7.5T / (237.3+T))          [hPa]
較正気圧      P_cal = 1013.25/760 × (1 - 0.000182T) × P       [hPa]
空気密度      ρ     = 1.293 × (273.15/(273.15+T)) × (P_cal/1013.25) × (1 - 0.378e/P_cal)
動圧高さ      h     = (V - offset) × a + b                     [cm, 水柱]
風速          U     = √(2 × water_density × h × g / ρ)         [m/s]
```

`a`・`b` は 2022年3月29日 微差圧センサ風速較正値（yymmdd.xlsx より）。
校正定数はセンサや日によって変化しうるため、**校正のやり直しが必要な場合は
`fix_windspeed/`（[fix_windspeed_README.md](../fix_windspeed/fix_windspeed_README.md)）を参照**してください。
...
フラッター実験（`flutter_run_experiment.m`）のデータ（`_ofst` / `_cXX` 構成）の後処理
（`flutter_analysis.py` / `lco_analysis.py`）は、剛体翼とは別系統のため
**専用ドキュメント [FLUTTER_README.md](FLUTTER_README.md) にまとめている**（自動実行の流れ・
手動実行コマンド・出力グラフの種類と出し方・CLIオプションはそちらを参照）。
LCO 図・指標の読み方は [LCO_ANALYSIS_GUIDE.md](LCO_ANALYSIS_GUIDE.md)。
...
## python パッケージの手動インストール
```python
1. post_processディレクトリに移動します
cd <userのpath>\wind_tunnel_automation\post_process
...
4. requirements.txtの内容をインストールします（アクティベート状態であることを確認してください）
