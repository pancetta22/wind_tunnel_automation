...
### 風洞壁補正（`C_aero_raw.csv` → `C_aero.csv`）

`calc_force.py` の `calb()` が、開放型測定部・2次元翼のポテンシャル理論補正を適用する
（出典: Brooks & Marcolini AIAA Paper 84-2266 / AGARDograph 336 Sec.2 /
Barlow, Rae & Pope 1984）。c=0.20 m（翼弦長）、h=0.60 m（測定部高さ）。

| 量 | 補正式 | 意味 |
|---|---|---|
| 迎角 | AoA_mod = AoA − [(c/4h) + (π/24)(c/h)²]·Cl·(180/π) | 噴流たわみ＋流線曲率 |
| 抗力 | Cd_mod = Cd − (c/4h)·Cl² | ダウンウォッシュによる誘導成分 |
| Cm | Cm_mod = Cm − (π²/96)(c/h)²·Cl | 曲率の見かけキャンバー分 |
| 揚力 | Cl は無補正 | 横軸（迎角）側を補正する枠組み |

- Brooks & Marcolini の乗算形 α_eff = α/ζ（ζ = (1+2σ)² + √(12σ)、σ = (π²/48)(c/h)²）と
  (c/h)² のオーダーまで等価な加算形。
- Windy は c/h = 1/3 と大きく ζ ≈ 1.62 の大補正。前提は付着流（線形域）なので、
  失速域（|AoA| > 10° 程度）の補正値は目安。各点の実測 Cl で横軸をずらすため、
  Cl が荒れると AoA_mod は非単調になる（グラフが横に折り返して見える）。
- 比較パワポの Cl スロープ・α₀ は AoA_mod 基準。zero_lift_report.json（原点パルス推奨値）は
  機械角の話なので補正前の幾何迎角 AoA を使う。
- c・h・翼面積 S=0.04 m²・代表翼弦 0.20 m は `calc_force.py` にハードコード。
  模型を変える場合は書き換えが必要。
