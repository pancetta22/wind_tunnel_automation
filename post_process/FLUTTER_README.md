...
### 高速化：計測点の並列処理と軽量描画（既定で有効）

計測点（1条件あたり数十点の6軸CSV）の処理は互いに独立なので、既定で
`min(8, CPUコア数-2)` プロセスに分散する。加えて計測点ごとの図は既定で
軽量描画（時系列を min/max 間引き。波形の外形・スパイクは保たれる）になる。
数値出力（`flutter_summary.csv` の値・列順）はワーカー数・描画モードに
よらず常に同一。実測（62点・1条件・`--lco`）で **約3分 → 約30秒**。

- **トラブル時は `--workers 1`** を付けると、並列を使わない従来どおりの
  逐次実行に戻る（挙動の切り分けに使う。数値は並列時と完全一致）。
- **従来品質の図が必要なら `--hq`** を付ける（間引きなし・`bbox_inches="tight"`）。
- 並列化によりタスクマネージャ／`ps aux | grep python` に `python.exe` の
  子プロセスが複数（既定で最大8個）見えるのは正常な動作。**二重起動の
  確認は「同じ対象フォルダに対する python.exe の親プロセスが複数無いか」
  で判断する**こと（子ワーカーの数では判断しない）。
- Gドライブ直処理を避けるルールは並列化後さらに重要になる（同時書き込みの
  本数が増えるため）。上記のローカルコピー手順は必ず守る。
...
| `--workers`       | 計測点処理の並列ワーカー数。`1` で従来同様の逐次実行（トラブル時の切り分け用）。数値出力（`flutter_summary.csv`）はワーカー数によらず同一 | `min(8, CPUコア数-2)` |
| `--hq`            | 計測点ごとの図を従来品質で出力（間引きなし・`bbox_inches="tight"`）。既定は高速な軽量描画（時系列は min/max 間引き、波形の外形・スパイクは保存される） | 無効（軽量描画） |
